    ...
    def rename_opti_cols(self, x_list, y_dict=None):
        ...
        if y_dict:
            ...
            for i, y in enumerate(self.task_list):
                self.df_opti.columns = self.df_opti.columns.str.replace(f'__{i}', f'__{y}')
...
class Evaluation():
    def __init__(self, evaluation_base, task_constrains, opti_constrains, pareto_ascending):
        ...
        self.opti_traget = [key.split('__')[1] for key, item in self.task_constrains.items() if item == 'opti'][0]
        self.opti_x = list(set(self.base.task_list) - set([key.split('__')[-1] for key, item in self.task_constrains.items()]))[0]
    ...
    def get_optimisation_task(self):
        ...
        for key, item in self.task_constrains.items():
            ...
            if item != 'opti':
                self.datapoints = self.datapoints.loc[self.datapoints[key.split('__')[-1]]>=float(item)]           
    ...
    def drop_not_in_bounds(self, cols):
        for col in cols:
            self.df_task = self.df_task[self.df_task[f'model_output__{col}'] <= self.df_task[f'con__{col}']]
    ...
    def get_opti_datapoints(self):
        ...
        datapoints[f'cummax__{self.opti_traget}']= datapoints[self.opti_traget].cummax()
        datapoints = datapoints[datapoints[f'cummax__{self.opti_traget}'] == datapoints[self.opti_traget]]
        ...
        max_row[self.opti_x ] = self.df_task[f'con__{self.opti_x}'].max()
        datapoints = pd.concat([min_row, datapoints, max_row], ignore_index=True)
        for domain in ['model_output__', 'sim_output__', 'con__']:
        ...
        with open(os.devnull, "w") as f, contextlib.redirect_stdout(f):    
            for index, exp in self.exp_overview.loc[self.filter_index(model_substrings=['Sim', 'database'], inclusive=False)].iterrows():
                for task in task_list:
                    exp['data'] = exp['data'].dropna(subset=[f'sim_output__{task}', f'model_output__{task}'])
                    model_pred_power = regression_results(exp['data'][[f'sim_output__{task}']], exp['data'][[f'model_output__{task}']])
                    model_pred_power.extend([exp['model_name'], task, exp['optimizer_name']])
                    print(model_pred_power)
                    print(self.scores)
    ...
    def calc_cum_max_optimizer(self, cols=None):
        ...
        for index, exp in self.exp_overview.iterrows():
            df = exp['data']
            for col in cols:
                df = df.sort_values(by=[f'con__{self.opti_x}'], ascending=self.pareto_ascending)
                df[f'cummax__{col}'] = df[col].cummax().clip(lower=0)
    ...
    def calc_cum_max(self, cols=None):
        if cols is None:
            cols = [f'model_output__{self.opti_traget}', f'sim_output__{self.opti_traget}']
        # Calc cum max for each model + (database)
        for index, exp in self.exp_overview.iterrows():
            df = exp['data']
            for col in cols:
                model_type = col.split('_')[0]
                for con in df[f'con__{self.opti_x}'].unique():
                    df = df.sort_values(by=[f'con__{self.opti_x}'], ascending=self.pareto_ascending)
                    if self.pareto_ascending:
                        df.loc[df[f'con__{self.opti_x}']==con, f'cummax__{col}'] = df.loc[df[f'{model_type}_output__{self.opti_x}']<=con, col].clip(lower=0).max()
                    else:
                        df.loc[df[f'con__{self.opti_x}']==con, f'cummax__{col}'] = df.loc[df[f'{model_type}_output__{self.opti_x}']>=con, col].clip(lower=0).max()
            self.exp_overview.at[index, 'data'] = df
    ...
    def calc_normalized_score(self, cols=None, sim='all'):
        ...
        if sim == 'all':
            sim = self.base.df_opti.loc[(self.base.df_opt['model_name'].str.contains('Sim')) & (self.base.df_opt[f'con__{self.opti_x}']!='opti')]
            sim.astype({f'con__{self.opti_x}': 'float'})
        if sim == 'task':
        ...
        for index, exp in self.exp_overview.loc[self.filter_index(model_substrings=['Sim', 'database'], inclusive=False)].iterrows():
            ...
            for con in df[f'con__{self.opti_x}'].unique():
                for col in cols:
                    ...
                    sim_column = sim[f'model_output__{self.opti_traget}']
                    
                    if self.pareto_ascending:
                        data_query = data[f'con__{self.opti_x}'] <= con
                        sim_query = sim[f'con__{self.opti_x}'] <= con
                    else:
                        data_query = data[f'con__{self.opti_x}'] >= con
                        sim_query = sim[f'con__{self.opti_x}'] >= con
                    ...
                    df.loc[df[f'con__{self.opti_x}']==con, f'normed__{col}'] = min_max_scaling(df.loc[df[f'con__{self.opti_x}']==con, f'cummax__{col}'], data_value, max_value)
                    #df[f'normed_{col}'].where(df[f'normed_{col}'] < 1, 1, inplace=True)
            df.replace([np.inf, -np.inf], np.nan, inplace=True)
            self.exp_overview.at[index, 'data'] = df
 
    def calc_opti_score(self, cols=None, con_col = 'con__q', models=['Sim']):
        if cols is None:
            cols = [f'model_output__{self.opti_traget}']
        # Option if df_opti or df shall be used (optimal sim results with same opti options --> df, best results over all options --> df_opti)
        ...
        for index, exp in self.exp_overview.iterrows():
            ...
            for con in df[con_col].unique():
                for col in cols:
                    ...
                    min_value = 0
                    if self.pareto_ascending:
                        max_value = sim[f'y'].loc[(sim[f'con__{self.opti_x}']<=con)].clip(lower=0).max()
                    else:
                        max_value = sim[f'y'].loc[(sim[f'con__{self.opti_x}']>=con)].clip(lower=0).max()
                    if max_value == 0:
                        df.loc[df[f'con__{self.opti_x}']==con, f'normed_opti__{col}'] = 0
                    else:
                        # Use cummax?
                        df.loc[df[f'con__{self.opti_x}']==con, f'normed_opti__{col}'] = min_max_scaling(df.loc[df[f'con__{self.opti_x}']==con, col], min_value, max_value)
            df.replace([np.inf, -np.inf], np.nan, inplace=True)
            self.exp_overview.at[index, 'data'] = df
    ...
    def plot_2d_pareto(self, df=None, x=None, y_list=None, label=None, color_dict=ml_color_dict, save_fig = None):
        ...
        datapoints =  self.datapoints[(self.datapoints[x.split('__')[-1]] >= 0) & (self.datapoints[y_list[0].split('__')[-1]] >= 0)]
        params = {}
        fig, ax = plt.subplots()
        ...
        dict_ = {f'cummax__model_output__{self.opti_traget}':1.0 , f'cummax__sim_output__{self.opti_traget}':1}

        ax.plot(datapoints[x.split('__')[-1]], 
                 datapoints[y_list[0].split('__')[-1]],
                 c='grey',
                 #label = 'database',
                 marker='.',
                 linestyle=''
    ...
    def plot_summarize(self, df=None, targets=None, group='model_name', plot_dict=box_plot_dict, print_error=False, save_fig=False, y_range=None):
        if targets is None:
            targets = [f'normed__sim_output__{self.opti_traget}', f'normed__model_output__{self.opti_traget}']
        ...
        if print_error:
            for model in df_plot.model_name.unique():
                ...
                RMSE = round(self.scores.loc[(self.scores['model'] == model) & (self.scores['task'] == targets[0].split('__')[-1]) 
                                            & (self.scores['optimizer'] == optimizer), 'rmse'].item(),2)
