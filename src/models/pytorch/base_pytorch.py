...
class BaseModelPytorch(ModelBase, nn.Module):
    ...
    def tune(self):
        ...
        n_trials_per_process = config.OPTUNA_TRAILS // config.NUM_CORES

        def run_trail():
            study = optuna.load_study(study_name=study_name, storage=storage)
            study.optimize(self.objective, n_trials=n_trials_per_process)
        # Use multiprocessing for parallel execution
        processes = []
        for _ in range(config.NUM_CORES):
            p = multiprocessing.Process(target=run_trail)
            p.start()
            processes.append(p)
